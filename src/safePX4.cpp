 ...
 */
void dynamicReconfigureCallback(controllers::setSafePX4Config &config, uint32_t level){
 ...
 */
void commandPositionCallback(const geometry_msgs::PoseStamped& command_msg){
    float x = bound(command_msg.pose.position.x, MAX_X);
    float y = bound(command_msg.pose.position.y, MAX_Y);
    float z = bound(command_msg.pose.position.z, MAX_Z);
    float psi = bound(command_msg.pose.orientation.z, M_PI);
 ...
 */
void commandVelocityCallback(const geometry_msgs::TwistStamped& command_msg){
    float vx = bound(command_msg.twist.linear.x, MAX_V);
    float vy = bound(command_msg.twist.linear.y, MAX_V);
    float vz = bound(command_msg.twist.linear.z, MAX_V);
 ...
 */
void commandAttitudeCallback(const controllers::RollPitchYawThrust& command_msg){
    float roll = bound(-command_msg.pitch, MAX_RP);
    float pitch = bound(command_msg.roll, MAX_RP);
    float yaw = bound(command_msg.yaw, M_PI);
    float thrust = bound(command_msg.thrust, MAX_V);
...
// Constructor
SafePX4::SafePX4(int argc, char** argv){
    ros::init(argc, argv, "safePX4");
    ros::NodeHandle node_handle;

    // Subscribers
    optitrack_subscriber = node_handle.subscribe("/optitrack/odometry", 1, optitrackCallback);
    command_subscriber = node_handle.subscribe("/px4/command", 1, commandCallback);
    command_position_subscriber = node_handle.subscribe("/px4/command_position", 1, commandPositionCallback);
    command_velocity_subscriber = node_handle.subscribe("/px4/command_velocity", 1, commandVelocityCallback);
    command_attitude_subscriber = node_handle.subscribe("/px4/command_attitude", 1, commandAttitudeCallback);
    noise_subscriber = node_handle.subscribe("/px4/noise", 1, noiseCallback);
...
void SafePX4::run(){
    ...
    dynamic_reconfigure::Server<controllers::setSafePX4Config> server;
    dynamic_reconfigure::Server<controllers::setSafePX4Config>::CallbackType f;
...
int main(int argc, char** argv){
    cout << "[SafePX4] SafePX4 is running..." << endl;

    SafePX4* px4 = new SafePX4(argc, argv);
